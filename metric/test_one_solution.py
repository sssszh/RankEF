...
def check_correctness(prob_path, generation, timeout, debug , example_tests):
    ...
    def _temp_run(prob_path, generation, debug,example_tests, result_result,result_error,result_code):
        ...
        result_code.append(tmp[3])
    manager = multiprocessing.Manager()
    result_result = manager.list()
    result_error = manager.list()
    result_code = manager.list()
    p = multiprocessing.Process(target=_temp_run, args=(prob_path, generation, debug, example_tests,result_result,result_error,result_code))
    p.start()
    ...
    if not result_result:
        ...
        result_result = [-1]
        result_error = ["global timeout"]
        if debug:
            print(f"global timeout")
        return result_result,result_error
    result_error = list(result_error)
    result_code = list(result_code)
    return result_result,result_error,result_code
...
def eval_and_save_problems(args):
    ...
    for o_idx, o in tqdm(enumerate(gen_codes), total=len(gen_codes), ncols=0, leave=False):   
        ...
        curr_codes = []
        try:
            curr_results, curr_errors, curr_codes = check_correctness(prob_path=problem,generation=o,timeout=TIMEOUT, debug=args.debug, 
    ...
    save_results = {real_index : {'results': all_results, 'error': all_errors, 'sols': all_sols}} 
    # save_results = {real_index : {'results': all_results, 'error': all_errors}} 
