    ...
    if test is None:
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            try:
                ...
                error_info={"error": "SyntaxError", 
                "message": str(e), 
                "lineno": e.lineno, 
                "error_code": sol.split("\n")[e.lineno - 1],
                "traceback": tb}
                signal.alarm(0)
                if debug: 
                ...
                tb = traceback.format_exc()
                error_info={"error": e.__class__.__name__, 
                "message": str(e), 
                "lineno": "None",
                "error_code": "None",
                "traceback": tb}
...
###########生成由代码行组成的列表#####################
            new_test = []
            for x in tmp_test:   # 这里就是要得到一行一行的代码组成的列表
                if (not x.startswith("from ")) and (not x.startswith("import ")):  # 如果这一行代码不是import和from开头的话 # not x.startswith("from "):True   not x.startswith("import "):True
                    new_test.append("\t" + x + "\n")   # 加入\t是为了将其包裹在函数中
                else:
                    new_test.append(x + "\n")
            tmp_test = new_test   # tmp_test：由代码行组成的列表
#############把生成的代码包裹到函数当中################            
            new_test = ""
            started = False
            for i in tmp_test:
        ...
        for index, inputs in tqdm(enumerate(in_outs["inputs"]), total=len(in_outs["inputs"]), ncols=0, leave=False): 
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                ...
                try:
                    ...
                    intent_error = {"error": "OutputMismatch", 
                                    "message": f"Expected {expected_output}, but got {code_output}"}
                    ...
                    tb = traceback.format_exc()
                    tb_list = tb.split("\n")
                    error_line = tb_list[-3]  # 错误行信息位于traceback的倒数第三行
                    try: 
                        error_lineno = error_line.split(", ")[1].split(" ")[1]  # 从错误行信息中提取行号
                        error_code = sol.split("\n")[int(error_lineno) - 1]
                    except:
                        error_lineno = "-100"
                        error_code = "None"
