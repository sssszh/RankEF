...
from models.ModelBasedCodeT5Encoder import T5EncoderModel
import json
from transformers import RobertaTokenizer
tokenizer = RobertaTokenizer.from_pretrained("Salesforce/codet5p-770m-py")

model = T5EncoderModel.from_pretrained("path to trained model")
...
for fname in tqdm(prob_list, ncols=0, total=len(prob_list)):
    ...
    for idx, code_str in tqdm(enumerate(code_list), ncols=0, total=len(code_list)):
        ...
        # class_labels = torch.LongTensor([1]).unsqueeze(0)
        output = model(input_ids=input_ids, labels=class_labels, return_dict=False)
