...
class Trainer:
    ...
    def __init__(
        ...
        model_soft: Union[PreTrainedModel, nn.Module] = None,
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters.extend([
                {
                    "params": [
                        p for n, p in self.model_soft.named_parameters() if (n in decay_parameters and p.requires_grad)
                    ],
                    "weight_decay": self.args.weight_decay,
                },
                {
                    "params": [
                        p for n, p in self.model_soft.named_parameters() if (n not in decay_parameters and p.requires_grad)
                    ],
                    "weight_decay": 0.0,
                },
            ])
    ...
    def _inner_training_loop(
        ...
        model_soft = self._wrap_model(self.model_soft)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step = self.training_step(model, model_soft, inputs)
    ...
    def training_step(self, model: nn.Module, model_soft: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        """
        ...
        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, model_soft, inputs)
    ...
    def compute_loss(self, model, model_soft, inputs, return_outputs=False):
        """
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        cls_inputs = {'input_ids': inputs['class_input_ids'], 'class_labels': inputs['class_labels']}
        gen_inputs = {'input_ids': inputs['input_ids'], 'labels': inputs['labels']}
        cls_outputs = model(**cls_inputs)
        gen_outputs = model_soft(**gen_inputs)
        ...
        if labels is not None:
            ...
            if model_name in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
                loss = self.label_smoother(gen_outputs, labels, shift_labels=True)
            else:
                loss = self.label_smoother(gen_outputs, labels)
        else:
            if isinstance(gen_outputs, dict) and "loss" not in gen_outputs:
                raise ValueError(
                    "The model did not return a loss from the inputs, only the following keys: "
                    f"{','.join(gen_outputs.keys())}. For reference, the inputs it received are {','.join(inputs.keys())}."
                )
            # We don't use .loss here since the model may return tuples instead of ModelOutput.
            gen_loss = gen_outputs["loss"] if isinstance(gen_outputs, dict) else gen_outputs[0]
            cls_loss = cls_outputs["loss"] if isinstance(cls_outputs, dict) else cls_outputs[0]

            soft_sharing_loss = 0.0
            for param1, param2 in zip(model.encoder.parameters(), model_soft.encoder.parameters()):
                soft_sharing_loss += torch.norm(param1 - param2, p='fro')
            loss = self.alpha*cls_loss + (1-self.alpha)*gen_loss + soft_sharing_loss
            

        return (loss, cls_outputs) if return_outputs else loss
    
    def compute_class_loss(self, model, inputs, return_outputs=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.

        Subclass and override for custom behavior.
        """
        if self.label_smoother is not None and "class_labels" in inputs:
            class_labels = inputs.pop("class_labels")
        else:
            class_labels = None
        curr_inputs = {'input_ids': inputs['class_input_ids'], 'class_labels': inputs["class_labels"]}
        ...
        if class_labels is not None:
            if unwrap_model(model)._get_name() in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
                loss = self.label_smoother(outputs, class_labels, shift_labels=True)
            else:
                loss = self.label_smoother(outputs, class_labels)
        else:
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        loss, outputs = self.compute_class_loss(model, inputs, return_outputs=True)
                    loss = loss.mean().detach()

                    if isinstance(outputs, dict):
                        logits = tuple(v for k, v in outputs.items() if k not in ignore_keys + ["loss"] + ["encoder_last_hidden_state"])
                    else:
                        logits = outputs[1:]
                else:
                    loss = None
